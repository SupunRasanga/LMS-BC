...
export class IssueBookService {
  ...
  create(book: any): Observable<any>{
    ...
    return this.http.post(`${environment.baseAPIUrl}/issue-book.json`,book);
  }

  getAll(): Observable<any>{
    //return this.http.get(`${this.baseAPIUrl}/customer.json`).pipe(map((res) => {
    return this.http.get(`${environment.baseAPIUrl}/issue-book.json`).pipe(map((res) => {
      const books:any[] = [];
  ...
  update(book: any, id: string): Observable<any>{
    ...
    return this.http.put(`${environment.baseAPIUrl}/issue-book/${id}.json`,book);
  }

  delete(id: string): Observable<any>{
    //return this.http.delete(`${this.baseAPIUrl}/book/${id}.json`);
    return this.http.delete(`${environment.baseAPIUrl}/issue-book/${id}.json`);
  }
